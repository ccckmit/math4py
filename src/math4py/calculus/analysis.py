...
def limit(f: Callable, x0: float, direction: str = "both", h: float = 1e-6) -> Tuple[float, str]:
...
def is_continuous(f: Callable, x0: float, h: float = 1e-6, tol: float = 1e-6) -> bool:
    """檢查函數在 x0 是否連續。
...
def is_uniformly_continuous(
    f: Callable, interval: Tuple[float, float], n_samples: int = 100, tol: float = 1e-6
) -> bool:
    """檢查函數在區間上是否一致連續。
...
def pointwise_convergence(
    f_n: List[Callable], f: Callable, x_range: Tuple[float, float], n_samples: int = 50
) -> float:
    """檢查函數序列是否點態收斂。
...
def intermediate_value_theorem(
    f: Callable, a: float, b: float, tol: float = 1e-6
) -> Tuple[bool, float]:
...
def mean_value_theorem(f: Callable, a: float, b: float, f_prime: Callable) -> Tuple[bool, float]:
...
def extreme_value_theorem(
    f: Callable, interval: Tuple[float, float], n_samples: int = 1000
) -> Tuple[float, float]:
...
def riemann_integral(f: Callable, a: float, b: float, n: int = 1000) -> float:
    """黎曼積分（數值逼近）。
...
def sequence_limit(
    a_n: List[float], method: str = "definition", eps: float = 1e-6
) -> Tuple[float, bool]:
    """計算數列極限 lim_{n→∞} a_n。

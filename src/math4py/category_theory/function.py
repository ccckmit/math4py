...
def natural_transformation(F: Dict, G: Dict, components: Dict[Any, str]) -> Dict:
    """自然變換 η: F ⇒ G。
...
def limit_product(objects: List[Any], projections: Dict[Any, List[str]]) -> Dict:
    """極限：乘積 ∏ A_i。
...
def colimit_coproduct(objects: List[Any], injections: Dict[Any, List[str]]) -> Dict:
    """餘極限：餘乘積 ∐ A_i。"""
    return {"coproduct_object": "Q", "injections": injections, "universal": True}


def adjoint_functors(F: Dict, G: Dict, unit: Dict, counit: Dict) -> Dict:
    """伴隨函子 F ⊣ G。

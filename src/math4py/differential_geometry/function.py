...
def riemann_curvature_tensor(g: np.ndarray, Gamma: np.ndarray, d: int) -> np.ndarray:
    """計算黎曼曲率張量 R^i_jkl。
        ...
        for j in range(d):
            for k in range(d):
                for l in range(d):
                    ...
                    # Γ^i_mk Γ^m_jl
                    term3 = sum(Gamma[i, m, k] * Gamma[m, j, l] for m in range(d))
                    # Γ^i_ml Γ^m_jk
                    term4 = sum(Gamma[i, m, l] * Gamma[m, j, k] for m in range(d))
...
def lie_derivative(
    vector_field: np.ndarray, diff_vector_field: np.ndarray, coord_diff: np.ndarray, d: int
) -> np.ndarray:
    ...
    for i in range(d):
        for j in range(d):
            lie_deriv[i] += (
                vector_field[j] * coord_diff[i, j] - diff_vector_field[j] * coord_diff[j, i]
            )
...
def covariant_derivative(
    vector: np.ndarray, Gamma: np.ndarray, coord_diff: np.ndarray, d: int
) -> np.ndarray:
...
def metric_tensor_sphere(R: float = 1.0) -> Callable:
    ...
    def g(theta_phi: np.ndarray) -> np.ndarray:
        theta = theta_phi[0]
        # g = [[R², 0], [0, R² sin²θ]]
        metric = np.array([[R**2, 0.0], [0.0, (R * np.sin(theta)) ** 2]])
        return metric

    return g


def geodesic_distance_sphere(point1: np.ndarray, point2: np.ndarray, R: float = 1.0) -> float:
    """球面上兩點的大圓距離。"""
    # 簡化：假設點是 (θ, φ) 坐標
    ...
    # 球面餘弦定律
    cos_dist = np.sin(theta1) * np.sin(theta2) * np.cos(phi2 - phi1) + np.cos(theta1) * np.cos(
        theta2
    )

...
def stokes_theorem_check(
    F: Callable, curl_F: Callable, surface_vertices: list, n_steps: int = 100
) -> dict:
...
def divergence_theorem_check(
    F: Callable, div_F: Callable, volume_vertices: list, n_steps: int = 100
) -> dict:
...
def ricci_tensor_trace(Ric: np.ndarray, g_inv: np.ndarray, d: int) -> dict:
    ...
    return {"pass": True, "scalar_curvature": scalar_curv}

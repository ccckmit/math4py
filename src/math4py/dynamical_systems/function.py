...
def phase_space_trajectory(
    f: Callable, y0: np.ndarray, t_span: Tuple[float, float], n_steps: int = 1000
...
def fixed_point_analysis(
    f: Callable, y0: np.ndarray, tol: float = 1e-6, max_iter: int = 10000
) -> np.ndarray:
    """尋找不動點 f(y) = 0（牛頓法）。"""
    y = y0.copy()
...
def lyapunov_exponent(
    f: Callable, y0: np.ndarray, t_span: Tuple[float, float], n_steps: int = 10000
) -> float:
    """計算最大李雅普諾夫指數（簡化版）。"""
    t = np.linspace(t_span[0], t_span[1], n_steps)
...
def lorenz_system(
    y: np.ndarray, t: float, sigma: float = 10.0, rho: float = 28.0, beta: float = 8.0 / 3.0
) -> np.ndarray:
...
def bifurcation_diagram(
    r_range: Tuple[float, float], n_r: int = 100, n_transient: int = 100, n_plot: int = 100
) -> Tuple[np.ndarray, np.ndarray]:
    """分岔圖數據（簡化版）。"""

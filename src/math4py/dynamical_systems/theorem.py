...
def existence_uniqueness_theorem(
    f: Callable, y0: np.ndarray, t_span: tuple, eps: float = 1e-6
) -> bool:
...
def conservation_law_check(
    f: Callable, y0: np.ndarray, t_span: tuple, conservation_fn: Callable, n_steps: int = 1000
) -> dict:
    """檢查保守系統的守恆量。"""
...
def chaos_sensitivity_check(
    logistic_map_fn: Callable, x0: float, delta: float = 1e-10, n_steps: int = 100
) -> dict:

...
def inner_product_H(
    f: Callable, g: Callable, a: float, b: float, weight: Callable = None, n: int = 1000
) -> float:
...
def proj_orthogonal_H(
    f: Callable, e: Callable, a: float, b: float, weight: Callable = None, n: int = 1000
) -> float:
...
def gram_schmidt_H(
    functions: List[Callable], a: float, b: float, weight: Callable = None, n: int = 1000
...
def legendre_polynomials(n_max: int) -> List[Callable]:
    ...
    polys = [lambda x: np.ones_like(x), lambda x: x.copy()]
    for n in range(2, n_max + 1):

        def Pn(x, n=n, polys=polys):
            return (2 * n - 1) / n * x * polys[n - 1](x) - (n - 1) / n * polys[n - 2](x)

        polys.append(Pn)
    return polys[: n_max + 1]


def is_complete_basis_H(
    basis: List[Callable],
    ...
    for f in test_fns:
        ...
        for e in basis:
            ...
            def f_proj(x, old=old_f_proj, add=add_proj):
                return old(x) + add(x)
...
def riesz_representation(phi: Callable, basis: List[Callable], a: float, b: float) -> Callable:

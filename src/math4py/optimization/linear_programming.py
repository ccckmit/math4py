...
def simplex_method(
    c: np.ndarray, A: np.ndarray, b: np.ndarray, max_iter: int = 1000, tol: float = 1e-10
    ...
    for iteration in range(max_iter):
        ...
        ratios = [x_B[i] / d[i] if d[i] > tol else float("inf") for i in range(len(basic_orig))]
        leaving_idx = int(np.argmin(ratios))
...
def duality_gap(
    c: np.ndarray, x_primal: np.ndarray, lambda_dual: np.ndarray, A: np.ndarray, b: np.ndarray
) -> float:

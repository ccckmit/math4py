...
class Tensor:
            ...
            def grad_fn(grad):
                if self.requires_grad:
                    if axis is None:
                        ...
                        grad_expanded = np.ones_like(self.data) * grad / self.data.shape[axis]
                    else:
                        grad_expanded = np.zeros_like(self.data)
                        np.add.at(grad_expanded, (slice(None),)*(axis) + (np.newaxis,), grad / self.data.shape[axis])
                    self.grad = grad_expanded.copy() if self.grad is None else self.grad + grad_expanded
            result._grad_fn = grad_fn
    ...
    def __pow__(self, exponent):
        ...
        if isinstance(exponent, (int, float)):
            ...
            if result.requires_grad:
                def grad_fn(grad):
                    if self.requires_grad:
                        grad_pow = grad * exponent * (self.data ** (exponent - 1))
                        self.grad = grad_pow.copy() if self.grad is None else self.grad + grad_pow
                result._grad_fn = grad_fn

            return result
        elif isinstance(exponent, Tensor):
            result_data = self.data ** exponent.data
            result = Tensor(result_data, requires_grad=self.requires_grad or exponent.requires_grad)
            ...
            if result.requires_grad:
                def grad_fn(grad):
                    if self.requires_grad:
                        ...
                        self.grad = grad_self.copy() if self.grad is None else self.grad + grad_self
                    if exponent.requires_grad:
                        grad_exp = grad * (self.data ** exponent.data) * np.log(self.data)
                        exponent.grad = grad_exp.copy() if exponent.grad is None else exponent.grad + grad_exp
                result._grad_fn = grad_fn
    ...
    def softmax(self, axis=-1):
        ...
        if result.requires_grad:
            def grad_fn(grad):
                if self.requires_grad:
                    sum_grad_y = np.sum(grad * result_data, axis=axis, keepdims=True)
                    grad_softmax = result_data * (grad - sum_grad_y)
                    self.grad = grad_softmax.copy() if self.grad is None else self.grad + grad_softmax
            result._grad_fn = grad_fn

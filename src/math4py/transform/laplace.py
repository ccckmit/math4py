...
def inverse_laplace_bromwich(F: Callable, t: float, sigma: float = 1.0, 
                             omega_max: float = 100.0, domega: float = 0.1) -> float:
...
def convolution_theorem_laplace(f: Callable, g: Callable, t: float, s: complex) -> Tuple[complex, complex]:
    ...
    L_conv = laplace_transform(lambda tau: convolution(tau) if isinstance(tau, (int, float)) else 
                        np.array([convolution(t_i) for t_i in tau]), s)

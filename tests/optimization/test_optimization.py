...
class TestIsConvexFunction:
    def test_convex_quadratic(self):
        ...
        def f(x):
            return x**2 if isinstance(x, (int, float)) else x[0] ** 2
...
class TestHillClimbing:
    def test_quadratic_minimum(self):
        ...
        x0 = np.array([5.0])
        x_opt, f_opt, n_iter = hill_climbing(f, x0, step_size=0.5, maximize=False, max_iter=100)
    ...
    def test_quadratic_maximum(self):
        ...
        x0 = np.array([3.0])
        x_opt, f_opt, n_iter = hill_climbing(f, x0, step_size=0.5, maximize=True, max_iter=100)
        assert abs(x_opt[0]) < 0.5
        assert f_opt > -1.0
    ...
    def test_2d_gaussian_max(self):
        ...
        x0 = np.array([0.0, 0.0])
        x_opt, f_opt, n_iter = hill_climbing(f, x0, step_size=0.3, maximize=True, max_iter=200)
...
class TestHillClimbingSimple:
    def test_simple_quadratic(self):
        ...
        x0 = np.array([3.0])
        x_opt, f_opt, n_iter = hill_climbing_simple(f, x0, step_size=0.2, maximize=False)
    ...
    def test_2d_function(self):
        ...
        x0 = np.array([0.0, 0.0])
        x_opt, f_opt, n_iter = hill_climbing_simple(f, x0, step_size=0.3, maximize=False)
...
class TestRandomRestartHillClimbing:
    def test_with_restarts(self):
        ...
        bounds = [(0, 5), (0, 5)]
        x_best, f_best = random_restart_hill_climbing(f, bounds, n_restarts=5, maximize=True)
    ...
    def test_converges_to_global(self):
        ...
        bounds = [(-5, 5)]
        x_best, f_best = random_restart_hill_climbing(f, bounds, n_restarts=3, maximize=True)
...
class TestSimulatedAnnealing:
    def test_quadratic_min(self):
        ...
        x0 = np.array([5.0])
        bounds = [(-10, 10)]
        x_opt, f_opt, n_iter = simulated_annealing(
            f, x0, bounds, temperature=1.0, maximize=False, max_iter=2000
        )
    ...
    def test_escapes_local_minimum(self):
        ...
        bounds = [(-5, 5)]
        x_opt, f_opt, n_iter = simulated_annealing(
            f, x0, bounds, temperature=10.0, cooling_rate=0.9, maximize=False, max_iter=2000
        )
        # 應該能逃離 x=-2 的局部最優
        assert f_opt < 1.0

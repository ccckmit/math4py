...
class TestContinuityEquation:
    def test_continuity_holds(self):
        # ρ1A1v1 = ρ2A2v2
        assert fd.continuity_equation(1.0, 1.0, 2.0, 1.0, 0.5, 4.0) == True

    def test_continuity_fails(self):
        # 不相等時應返回 False
        assert fd.continuity_equation(1.0, 1.0, 2.0, 1.0, 1.0, 3.0) == False


class TestBernoulliEquation:
    def test_bernoulli_holds(self):
        # 簡單情況：靜止流體 P + ρgh
        assert fd.bernoulli_equation(101325, 1000, 0, 10, 101325 + 1000*9.81*10, 0, 0) == True
